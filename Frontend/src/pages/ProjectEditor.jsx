...
export default function ProjectEditor({ projects, setProjects }) {
  ...
  const [filters, setFilters] = useState({
    Diffuser: true, Grille: true, Damper: true, Fan: true,
    VAV_FCU: true, AHU_RTU: true, Louver: true,
  })
  ...
  function startResize(type, e) {
    ...
    function onMove(ev) {
      ...
      if (type === "settings") {
        ...
        let newBoq = Math.max(0, Math.min(containerWidth - (mouseX + 8), containerWidth - handleWidth))
  ...
  return (
    <div ref={containerRef} className="flex flex-col h-full min-h-screen overflow-hidden">
      <EditorHeader projectName={project ? project.name : "Project"} onBack={() => navigate("/projects")} />
      ...
      <div className="flex flex-1 min-h-0 w-full overflow-hidden">
        ...
        <div
          ...
          className="w-2 bg-zinc-900 hover:bg-zinc-800 relative flex-shrink-0 z-50 transition-colors flex items-center justify-center cursor-col-resize"
        >
          <div className="grid grid-cols-2 gap-0.5">
             {[...Array(6)].map((_, i) => (
               <div key={i} className="w-0.5 h-0.5 bg-zinc-500 rounded-full"></div>
             ))}
          </div>
        </div>
